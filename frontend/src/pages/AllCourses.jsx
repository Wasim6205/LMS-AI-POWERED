...
const AllCourses = () => {
  ...
  return (
    <div className='flex min-h-screen bg-gray-50'>
        ...
        {/* sideBar */}
        <aside className={`w-[260px] h-screen overflow-y-auto bg-black fixed top-0 left-0 p-6 py-[130px] border-r border-gray-200 shadow-md transition duration-300 z-5 ${isSidebarVisible ? "translate-x-0" : "-translate-x-full"} md:block md:translate-x-0`}>
            <h2 className='text-xl font-bold flex items-center justify-center gap-2 text-gray-50 mb-6'><FaArrowLeftLong onClick={()=>navigate("/")} className={`text-white`} /> Filter by Category</h2>
            ...
            <form onSubmit={(e)=>e.preventDefault()} className='space-y-4 text-sm bg-gray-600 border-white text-[white] border p-[20px] rounded-2xl'>
                <button onClick={()=>navigate(`/search`)} className='px-[10px] py-[10px] bg-black text-white rounded-[10px] text-[15px] font-light flex items-center justify-center gap-2 cursor-pointer'>Search with AI <img src={ai} className='w-[30px] h-[30px] rounded-full' alt="" /></button>
        ...
        <main className={`w-full transition-all duration-300 py-[130px] md:pl-[300px] md:pl-[50px] flex items-start justify-center md:justify-start flex-wrap gap-6 px-[10px]`}>
            {
