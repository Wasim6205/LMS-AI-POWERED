    ...
    <div className='flex min-h-screen bg-gray-100'>
      ...
      <div className='flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-3'>
        <div className='flex items-center justify-center gap-3'>
          <FaArrowLeftLong onClick={()=>navigate(-1)} className='w-[22px] h-[22px] cursor-pointer' />
          <h1 className='text-2xl font-semibold'>All Created Courses</h1>
        </div>
        <button onClick={()=>navigate('/createcourse')} className='bg-[black] cursor-pointer text-white px-4 py-2 rounded hover:bg-gray-500'>Create Course</button>
      </div>
        ...
        <table className='min-w-full text-sm'>
          ...
          <tbody>
            ...
            <tr key={index} className='border-b hover:bg-gray-50 transition duration-200'>
              ...
              <td className='px-4 py-3'><FaEdit onClick={()=>navigate(`/editcourse/${course?._id}`)} className='text-gray-600 hover:text-blue-600 cursor-pointer' /></td>
            </tr>
            ))}
          </tbody>
        </table>
