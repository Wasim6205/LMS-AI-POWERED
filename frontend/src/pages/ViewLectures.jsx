  ...
  return (
    <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
        ...
        <div className='w-full md:w-2/3 bg-white rounded-2xl shadow-md p-6 border border-gray-200'>
            <div className='mb-6'>
                <h2 className='text-2xl font-bold flex items-center justify-start gap-[20px] text-gray-800'><FaArrowLeftLong onClick={()=>navigate("/")} className='text-black w-[22px] h-[22px] cursor-pointer' /> {selectedCourse?.title}</h2>
                <div className='mt-2 flex gap-4 text-sm text-gray-500 font-medium'>
                    <span className=''>Category : {selectedCourse?.category}</span>
                    <span className=''>Level : {selectedCourse?.level}</span>
                </div>
            </div>
            {/* video player */}
