...
const MyEnrolledCourses = () => {
  ...
  return (
    <div className='min-h-screen w-full px-4 py-9 bg-gray-50'>
      <FaArrowLeftLong onClick={()=>navigate('/')} className='absolute top-[3%] md:top-[6%] left-[5%] w-[22px] h-[22px] cursor-pointer' />
                ...
                {userData?.enrolledCourses?.map((course,index) => (
                    <div key={index} className='bg-white rounded-2xl shadow-md overflow-hidden border'>
                        ...
                        <div className='p-4'>
                            ...
                            <h1 onClick={()=>navigate(`/viewlecture/${course._id}`)} className='px-[10px] text-center py-[10px] border-2 bg-black border-black text-white rounded-[10px] text-[15px] font-light cursor-pointer mt-[10px] hover:bg-gray-600'>Watch Now</h1>
                        </div>
                    </div>
                ))}
            </div>
